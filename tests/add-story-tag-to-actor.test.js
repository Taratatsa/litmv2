...
describe("addStoryTagToActor", () => {
	it("routes a hero's story tag through the backpack item, not the actor", async () => {
		...
		const actor = fakeActor({ type: "hero", system: { backpackItem: backpack } });
	...
	it("warns and bails when a hero has no backpack", async () => {
		...
		const result = await addStoryTagToActor(actor, storyTagEffect({ name: "x" }));

		expect(result).toBeUndefined();
		expect(ui.notifications.warn).toHaveBeenCalledWith("LITM.Ui.warn_no_backpack");
		expect(actor.createEmbeddedDocuments).not.toHaveBeenCalled();
	});
