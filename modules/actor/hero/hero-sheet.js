...
export class HeroSheet extends LitmActorSheet {
	...
	static async #onSubmitForm(event, form, formData) {
		...
		const toDelete = HeroSheet.#cleanupEmptyRelationships(
			this.document,
			submitData,
		);
	...
	static #cleanupEmptyRelationships(actor, submitData) {
		...
		const relationshipIds = new Set(
			actor.system.relationships.map((e) => e.id),
		);
		for (const key of Object.keys(submitData)) {
	...
	static async #onSelectTag(event, target) {
		...
		const isWeaknessTag =
			(tagFromSystem?.type ?? actionTarget.dataset.tagType) === "weakness_tag";
		...
		if (event.altKey) {
			...
			const effect = resolveEffect(tagFromSystem.id, this.document, {
				fellowship: true,
			});
			if (!effect) return;
