...
export class ThemeAdvancementApp extends foundry.applications.api.HandlebarsApplicationMixin(
	...
	static DEFAULT_OPTIONS = {
		...
		actions: {
			...
			activateSpecialImprovement:
				ThemeAdvancementApp.#onActivateSpecialImprovement,
		},
	};
	...
	async _prepareContext(_options) {
		...
		const inactiveSpecialImprovements = (
			theme.system?.specialImprovements || []
		)
			.map((improvement, index) => ({
				value: String(index),
				label:
					improvement.name || game.i18n.localize("LITM.Ui.improvement_name"),
			}))
