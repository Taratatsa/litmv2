...
export function ensureLegacyEffects(data) {
	...
	if (data.name) {
		data.effects.push({
			...
			system: {
				question: "0",
				isScratched: data.system?.isScratched ?? false,
				isTitleTag: true,
			},
		});
	}
...
export class HeroCreationData {
	...
	async ensureIndexes() {
		...
		this._cache.themebooks = await this.#loadPackIndex(
			"themebooks",
			"themebook",
			["name", "img", "type", "system.theme_level"],
		);
		...
		if (power.length) {
			sections.push(
				`<div class="tag-tooltip-group"><label>${t(
					...
					.map((n) => {
						const s = foundry.utils.escapeHTML(n);
						return `<span class="litm-power_tag" data-text="${s}">${s}</span>`;
					})
					.join(" ")}</div>`,
			);
		}
		if (weakness.length) {
			sections.push(
				`<div class="tag-tooltip-group"><label>${t(
					...
					.map((n) => {
						const s = foundry.utils.escapeHTML(n);
						return `<span class="litm-weakness_tag" data-text="${s}">${s}</span>`;
					})
					.join(" ")}</div>`,
			);
		}
	...
	getThemeTagOptions(themeDoc) {
		...
		if (legacy) {
			return {
				...
				weaknessTags: (legacy.weaknessTags ?? [])
					.map((t) => t.name)
					.filter(Boolean),
			};
		}
		...
		for (let i = 0; i < THEME_SLOTS; i++) {
			...
			if (theme.method === "themebook") {
				...
				if (!theme.name) {
					return {
						index: i,
						reason: "LITM.Ui.hero_creation_manual_name_required",
					};
				}
			}
					...
					if (hasPowerSelection || hasWeaknessSelection) {
						...
						data.effects = (data.effects || []).map((e) => {
							...
							if (hasWeaknessSelection && e.type === "weakness_tag") {
								return {
									...e,
									disabled: e.name !== themeState.selectedWeaknessTag,
								};
							}
							return e;
						});
